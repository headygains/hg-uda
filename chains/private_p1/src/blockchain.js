...
class Blockchain {
    ...
    constructor() {
        this.chain = [];
        this.height = -1;
        this.initializeChain().then(r => r).catch(e => alert(e));
    }
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            try{
                let cHeight = await self.getChainHeight();
                let pBlock= await self.getBlockByHeight(cHeight);
                if(pBlock !== null){
                    block.previousBlockHash = pBlock.hash;
                }
                block.height = cHeight += 1;
        ...
        return new Promise(async (resolve) => {
            ...
            for(let i = 0; i <= self.chain.length - 1; i += 1){
                ...
                if(self.chain[i].previousBlockHash !== self.chain[i-1].hash){
                    errorLog.push(`Previous block mis-match: ${self.chain[i].hash}`)
                }
            }
            resolve(errorLog);
        });
